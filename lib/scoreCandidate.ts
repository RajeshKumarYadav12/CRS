import { Candidate, JobData, RankedCandidate, RecruiterFilters } from '@/types';
...
export function scoreCandidate(
  ...
  const matchedRequired = required.filter(r => candidate.skills.includes(r));
  const matchedPreferred = preferred.filter(p => candidate.skills.includes(p));

  const matchedSkills = Array.from(new Set([...matchedRequired, ...matchedPreferred]));
  const missingSkills = required.filter(r => !candidate.skills.includes(r));
